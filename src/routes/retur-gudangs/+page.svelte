<script lang="ts">
	...
	async function fetchItems() {
		const response = await fetch(
			`/api/retur-gudangs?page=${currentPage}&limit=${limit}&search=${searchTerm}`
		);
	...
	async function deleteItem(id) {
		if (confirm('Are you sure you want to delete this item?')) {
			try {
				const response = await fetch(`/api/retur-gudangs?id=${id}`, {
					method: 'DELETE'
				});
				if (response.ok) {
					fetchItems();
	...
	async function saveEdit() {
		try {
			const response = await fetch(`/api/retur-gudangs?id=${editingItem._id}`, {
				method: 'PUT',
				headers: {
	...
	async function createItem() {
		try {
			const response = await fetch('/api/retur-gudangs', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
		...
		<div class="w-3/4">
			<div class="bg-white rounded shadow-md">
				<div class="flex items-center justify-between p-4 border-b">
					<span class="font-bold">Barang Retur Gudang</span>
					<button
					...
					<div class="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-4">
						{#each items as item}
							<div class="p-4 bg-white rounded-lg shadow-md hover:bg-gray-600 hover:text-white">
								...
								<p class="text-sm text-start">Yang Menerima: {item.yang_menerima}</p>
								<p class="text-sm text-start">Yang Membawa: {item.yang_membawa}</p>
	...
	>
		<div class="relative flex flex-col w-full max-w-md p-8 m-auto bg-white rounded-lg">
			...
			<input
				type="text"
				class="w-full px-3 py-2 mb-3 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
				placeholder="Yang Menerima"
				bind:value={newItem.yang_menerima}
			/>
			<input
				type="text"
				class="w-full px-3 py-2 mb-3 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
				placeholder="Yang Membawa"
				bind:value={newItem.yang_membawa}
			/>
			<input
				type="text"
				class="w-full px-3 py-2 mb-3 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
			...
			>
				...
				<div class="relative flex-auto p-6">
					<form>
						...
						<div class="mb-4">
							<label class="block mb-2 text-sm font-bold text-gray-700" for="yang_menerima">
								Yang Menerima
							</label>
							<input
								class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
								id="yang_menerima"
								type="text"
								bind:value={editingItem.yang_menerima}
							/>
						</div>
						<div class="mb-4">
							<label class="block mb-2 text-sm font-bold text-gray-700" for="yang_membawa">
								Yang Membawa
							</label>
							<input
								class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
								id="yang_membawa"
								type="text"
								bind:value={editingItem.yang_membawa}
							/>
							<div class="mb-4">
