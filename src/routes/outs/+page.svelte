<script lang="ts">
	...
	async function fetchItems() {
		const response = await fetch(
			`/api/outs?page=${currentPage}&limit=${limit}&search=${searchTerm}`
		);
	...
	async function deleteItem(id) {
		if (confirm('Are you sure you want to delete this item?')) {
			try {
				const response = await fetch(`/api/outs?id=${id}`, {
					method: 'DELETE'
				});
				if (response.ok) {
					fetchItems();
	...
	async function saveEdit() {
		try {
			const response = await fetch(`/api/outs?id=${editingItem._id}`, {
				method: 'PUT',
				headers: {
	...
	async function createItem() {
		try {
			const response = await fetch('/api/outs', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
		...
		<div class="w-3/4">
			<div class="bg-white rounded shadow-md">
				<div class="flex items-center justify-between p-4 border-b">
					<span class="font-bold">Barang Keluar</span>
					<button
